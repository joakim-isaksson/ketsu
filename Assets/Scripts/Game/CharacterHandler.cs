	...
	public class CharacterHandler : ExtendedMonoBehaviour
	{
		...
		void NextMoveAction ()
		{
			...
			if (ActiveCharacter.Type == MapObjectType.Ketsu) {
				...
				if (other != null)
					HandleMoveAction (direction, active, other);
				else
					HandleMoveAction (direction, active, null);
			}
		}
		...
		void HandleMoveAction (Vector3 direction, Character active, Character other)
		{
			...
			while (activeMoving || otherMoving) {
				...
				if (activeMoving)
					activeTarget = new TargetInfo (active, activePointer.Position + direction);
				if (otherMoving)
                ...
                // No blockers
					...
					if (activeTarget.Ground.Type != MapObjectType.Ice)
						activeMoving = false;
					if (other != null && otherTarget.Ground.Type != MapObjectType.Ice)
						otherMoving = false;
				}
                ...
                // Active is blocked
					...
					activeMoving = false;
					if (otherPointer != null && otherPointer.Ground.Type == MapObjectType.Ice)
						otherPointer = otherTarget;
					else
						otherMoving = false;
                ...
                // Other is blocked
					...
					otherMoving = false;
					if (activePointer.Ground.Type == MapObjectType.Ice)
						activePointer = activeTarget;
					else
						activeMoving = false;
		...
		class TargetInfo
		{
			...
			public TargetInfo (Character character, Vector3 position)
			{
				...
				foreach (MapObject obj in MapManager.GetObjects(position)) {
					if (Blocker == null && character.IsBlockedBy (obj))
						Blocker = obj;
					if (Ground == null && obj.Layer == MapObjectLayer.Ground)
						Ground = obj;
					if (Blocker != null && Ground)
						break;
				}
			}
		}
